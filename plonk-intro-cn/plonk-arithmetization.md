...
其中 $n$ 为乘法门的数量，而 $m$ 大致为引线的数量。每一个矩阵的第 $i$ 行「选择」了乘法门 $\#i$ 的输入输出变量。比如我们定义电路的左输入矩阵  $U$ ：
...
回看下例子电路，我们把三个门全都编号，$\#1,\#2,\#3$，同时把加法门的输出值也标记为变量 $x_6$。
...
在协议共识阶段，Prover 和 Verifier 一起对 $(Q,\sigma)$ 进行共识。
