...
把多个约束验证合并的神奇能力来自于「多项式随机挑战」。如果有两个多项式 $f(X)$ 和 $g(X)$ 同为两个次数不超过 $d$ 的多项式，$f,g\in\mathbb{F}_{<d}[X]$。那么 Verifier 只需要给出一个随机挑战值 $\zeta\in \mathbb{F}$，计算 $f(\zeta)$ 是否等于 $g(\zeta)$ 即可大概率得知 $f(X)=g(X)$，其中出错的概率 $\leq\frac{d}{|\mathbb{F}|}$。只要保证 $\mathbb{F}$ 足够大，那么检查出错的概率就可以忽略不计。
...
我们需要另一种多项式编码方案，Lagrange 插值。即构造多项式 $a(X)$，使得它在定义域 $H=(w_0, w_1, \ldots w_{n-1})$ 上的取值为 $\vec{a}$，即
$$
\begin{split}
a(w_0) &= a_0 \\
a(w_1) &= a_1 \\
...
对于任意的多项式 $f(X)$，$\deg(f)<n$，那么它都可以唯一地表示为：
