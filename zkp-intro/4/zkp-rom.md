...
除了采用「随机预言机」之外，还有一类非交互零知识证明系统采用「公共参考串」（Common Reference String），简称「CRS」。它是在证明者 Alice 在构造 NIZK 证明之前由一个受信任的第三方产生的随机字符串，CRS 必须由一个受信任的第三方来完成，同时共享给 Alice 和 验证者 Bob。

是的，你没看错，这里又出现了「第三方」！虽然第三方不直接参与证明，但是他要保证随机字符串产生过程的可信。这是另一种构建「信任根基」的方式。而产生 CRS 的过程也被称为「Trusted Setup」，这是大家又爱又恨的玩意儿。

基于「随机预言模型」的 NIZK 方案的一个很大的优势就是**不需要** Trusted Setup，但是代价就是产生的证明通常都不够**简洁**。最关键的一点是，这类 NIZK 方案的安全性依赖一个似乎很虚无缥缈的安全假设。

而「CRS」方案则一般不需要依赖随机预言模型，但是需要绕不开 Trusted Setup。显然，在现实场景中引入第三方会让人头疼。CRS 到底用来作什么？Trusted Setup 的信任何去何从？这部分内容将留给本系列的下一篇。
...
*致谢：特别感谢 丁晟超，刘巍然 的专业建议和指正，感谢安比实验室小伙伴们(p0n1, even, aphasiayc, Vawheter, yghu, mr)的修改建议。*
