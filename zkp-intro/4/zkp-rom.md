...
本文继续长篇大论零知识证明背后的机制原理。一点总结，希望帮助大家理解这一类「现代密码学工具」的大致轮廓。本文约8000字，少量数学公式。
...
我们之前介绍的零知识证明系统都是「交互式」证明系统，需要验证者 Bob 在交互中提供一个或若干个「随机数」来挑战，比如「地图三染色问题」（参看『系列二』）中验证者 Bob 需要「不断地」随机挑选一条边来挑战 Alice 的答案，直到 Bob 满意为止，而 Alice 作弊的概率会「指数级」地衰减。而让 Bob 相信「证明」的基础取决于 Bob 所挑选随机数是不是足够随机。如果 Alice 能够提前预测到 Bob 的随机数，灾难就会发生，现实世界就会退化成「理想世界」，而 Alice 就可以立即升级成「模拟器」，通过超能力来任意愚弄 Bob。

而『系列三』中，我们详细分析了 Schnorr 协议。在 Schnorr 协议中，虽然验证者 Bob 只需要挑选一个随机数 `c` 来挑战 Alice ，让她计算一个值 `z`。但是 Bob 绝对不能让 Alice 有能力来预测到 `c` 的信息，否则，Alice 也会变身成模拟器。

熟悉区块链技术的朋友们想必对随机数的重要性印象深刻。

> 通过随机数挑战是交互式零知识证明的「信任根基」。

但，「交互过程」会限制应用场景。如果能将交互式零知识证明协议变成一个非交互式的零知识证明协议？这会非常非常激动人心。所谓的非交互可以看成是只有「一轮」的证明过程，即Alice 直接发证明给 Bob，然后 Bob 进行验证。

非交互式零知识证明，英文是`Non-Interactive Zero Knowledge`，简称 NIZK。它意味着我们可以把证明整个编码为一个「字符串」，然后可以写到一张纸上，通过邮件、聊天工具等各种方式随意发送给多个验证者，证明字符串甚至可以放在 Github 上随时供大家下载验证。

在区块链时代，「NIZK」就可以作为共识协议的一部分。因为一个区块链交易**需要多个矿工进行校验**。设想下，如果交易的发送者和每一个矿工都要交互一下，让矿工进行挑战，那么共识协议将奇慢无比，实现几无可能。而非交互式零知识证明则**可以直接广播给所有的矿工节点**，让他们自行验证。

可能有朋友会问：只让一个矿工挑战不就够了吗？把矿工和交易发送者的交互脚本编码成证明，然后广播给其他矿工，然后其他矿工就直接相信这个挑战过程是可信的，不也可以吗？但是，很显然，这里要么需要引入一个矿工充当可信第三方，第三方？似乎不是一个好主意……

而非交互式零知识证明，以下我们直接说「NIZK」，似乎就很理想了，没有第三方赚差价。

## 「非交互」带来的困惑

非交互式零知识证明，NIZK，如果存在，那么它要比交互式证明强大地多。
...
我们已经回忆过「零知识」性质的证明（参考『系列二』），证明过程是需要构造一个模拟器（算法），它也和验证者（Bob）在理想世界中进行交互，而验证者 Bob 没有能力区分出来对方是否是真的 Alice 还是一个 Simulator 模拟器。
...
还有一个更深层次的问题，请大家回忆下「地图三染色问题」，之所以模拟器不能在「现实世界」中为非作歹，核心原因是，他在理想世界中有「时间倒流」的超能力，而在「现实世界」中不能存在这种黑魔法。现实世界的「不存在性」是关键。

而且，NIZK 中**没有交互**，于是还导致一个严重的后果，模拟器没有办法使用「时间倒流」这个超能力，当然似乎也就不能区分证明者在两个世界中的行为。
...
上面我们在分析的过程中，提到了交互挑战的缺失。确实，如果 Bob 不参与 Alice 产生证明的过程，证明所包含的每一个 Bit 都由 Alice 提供，似乎「证明」本身不存在任何让 Bob 信任的「根基」。这个似乎从「直觉」上说不通。
...
*注：在一个朴素模型下， 非交互式零知识证明能支持的语言只是平凡语言，隶属于 BPP，无法支持 NP 问题。而能证明 NP-Hard 问题的零知识证明系统才更有价值。*
...
其中 `R` 是 Alice 发给 Bob 的椭圆曲线点，`PK` 是公钥。大家可以好好看看这个利用 Hash 算法来计算 `c` 的式子。这个式子达到了两个目的：
...
而一个密码学安全 Hash 函数是「单向」的，比如 SHA256，SHA3，blake2 等等。这样一来，虽然 `c` 是 Alice 计算的，但是 Alice 并没有能力实现通过挑选 `c` 来作弊。因为只要 Alice 一产生 `R`，那么 `c` 就相当于固定下来了。我们假设 Alice 这个凡人在「现实世界」中是没有反向计算 Hash 的能力。
...
我们把上面这个方案稍微变下形，就得到了「数字签名」方案。所谓的数字签名，就是「我」向「你」出示一个字符串，比如「白日依山尽，黄河入海流」，然后为了证明这句诗是我出示的，我需要签署某样东西。这个东西能证明我的身份和这句诗进行关联。

## 从 NIZK 角度看数字签名

不严格地说，数字签名方案相当于在证明（1）我拥有私钥，并且（2）私钥和消息进行了关联计算。

我首先要证明我的身份，那么这个简单，这正是 Schnorr 协议的功能，就能够向对方证明「我拥有私钥」这个陈述。并且这个证明过程是零知识的：不泄露关于「私钥」的任何知识。
...
这里正是利用了我们前面讲过的 Hash 函数的单向性，从而保证了攻击者不能随意伪造签名。
...
前文提到，失去了挑战，似乎失去了证明的「信任根基」。而在 Schnorr 签名方案中，Hash 函数担负起了「信任根基」的角色。在这里，Hash 函数的角色有一个非常学术的名字：「随机预言机」（Random Oracle）。

可是这里为何用 Hash？实际上当 Alice 要产生公共随机数时，需要一个叫做「随机预言机」的玩意儿，这个东西到底是什么呢？

开脑洞时间到！

我们设想在「现实世界」中，天上的云彩里住着一位「精灵」，他手持一个双栏表格，左边一栏为字符串，右边一栏为数字。任何人，包括你我，包括 Alice 和 Bob，都可以发字符串给「精灵」。
...
大家会发现这个精灵的行为其实很像一个随机数发生器，但是又很不一样，因为当我们发送相同的字符串时，他会返回相同的数。这个精灵就是传说中的「随机预言机」。
...
而一个具有密码学安全强度的 Hash 函数「似乎」可以充当一个「伪」随机预言机。那么经过合并的安全协议需要额外增加一个很强的安全假设，这就是：
...
Schnorr 协议经过 Fiat-Shamir  变换之后，不仅变成了非交互，而且从 SHVZK 变成了 NIZK。SHVZK 需要要求验证者是诚实的，这个要求太强了。

*注：如果验证者 Bob 不诚实会怎样？那么 Bob 有可能抽取出 Alice 的知识。但是对于三步 Schnorr 协议而言，它是否满足「零知识」，目前还处于未知状态。我们在系列三中只证明了它满足一个比较弱的性质：SHVZK*。

但是，当 Schnorr 协议摇身一变，变成非交互零知识证明系统之后，就真正的「零知识」了。因为验证者不参与交互，所谓要求诚实的验证者这个问题就不复存在。

然而，可能你的问题也来了，这个论断听起来似乎有道理，请问能证明吗？

时间到了，“翠花，上模拟器”

怎么用模拟器大法来构造一个「理想世界」呢？大家可以想一下，我们之前使用过「时间倒流」，还有修改「随机数传送带」超能力来让「模拟器」来作弊。

可是现在协议没有交互了，这就意味着：「时间倒流」超能力不能用；Bob 的随机数传送带也不存在了，「篡改传送带」这个超能力也不能用！

> 但是模拟器总要具备某种「超能力」，从而能够构建信任的「根基」

（如果模拟器在没有超能力的情况下具备作弊能力，那相当于证明了协议的不可靠性）。

可能大家现在已经猜出来了，模拟器要在「随机预言机」上动手脚。

先考虑下构造一个「理想世界」来证明「零知识」。在理想世界中，模拟器「绑架」了负责提供预言的「精灵」，当 Bob 向精灵要一个随机数的时候，精灵并没有给一个真随机数，而是给一个 Zlice（模拟器假扮的 Alice）提前准备好的一个数（也符合一致性分布，保证不可区分性），「精灵」无可奈何地返回 Bob 一个看起来随机，但实际上有后门的数字。**所谓后门，就是这个数字是 Zlice 自己提前选择好的**。
...
接下来，那么怎么证明「可靠性」呢？在另一个「理想世界」中，一个叫做「抽取器」的玩意儿，也同样绑架了精灵。

当无辜 Alice 的向「精灵」要一个随机数时，「精灵」返回了一个 `c1`，「抽取器」从精灵的表格中偷窥到了`c1`，当 Alice 计算出来 `z1` 之后，然后这时候「抽取器」仍然可以发动「时间倒流」超能力，让 Alice 倒退到第二步，再次向「精灵」要一个随机数，Alice 发送的字符串显然和第一次发送的字符串是相同的，`(R, m)`。按道理，因为 `(R, m)` 已经写在精灵表格的「左栏」里，所以一个诚实的「精灵」应该返回`c1`。但是，「抽取器」绑架了精灵，他把表格中的对应 `(R, m)` 这一行的「右栏」改成了一个不同的数 `c2`。当 Alice 计算出另一个 `z2` 之后，抽取器就完成了任务，通过下面的方程计算出 Alice 的私钥 `sk`：
...
不仅仅对于 Schnorr 协议，对于任意的 「Public-Coin 协议」，都可以用 Fiat-Shamir 变换来把整个协议「压缩」成一步交互，也就是一个非交互式的证明系统，这个变换技巧来自于 Amos Fiat 与 Adi Shamir 两人的论文『How to Prove Yourself: Practical Solutions to Identification and Signature Problems.』，发表在1986年的 Crypto 会议上。
...
看上图，左边那个穿袍子的就叫 Merlin（魔法师梅林），中间拿剑的帅哥就是 King Arthur（亚瑟王），两个角色来源于中世纪欧洲传说——亚瑟王的圆桌骑士。

Arthur 是一个不耐烦的国王，他随身携带一个硬币，而 Merlin是一个有着无限制计算能力的神奇机器人魔法师（图灵机），然后魔法师想说服国王相信某个「论断」为真，于是魔法师会和国王进行到对话，但是由于国王比较懒，他每次只会抛一个硬币，然后「挑战」魔法师，而魔法师需要及时应对，而且需要让国王在 k 轮之后能够相信自己的论断。由于 Merlin 有魔法，所以亚瑟王抛的硬币都能被 Merlin 看到。

这与我们在『系列一』中提到的交互式证明系统（Interactive Proof System，简称 `IP`）有些神似，但又不同。`IP` 由 Goldwasser，Micali 与 Rackoff（简称GMR）在 1985 年正式提出，它的证明能力覆盖很大一类的计算复杂性问题。为什么称之为特殊，是因为在 `IP` 的定义中，证明者 Prover 和 验证者 Verifier 都是可以抛硬币的图灵机，Verifier 可以偷偷抛硬币，并对 Prover 隐藏。而在 Arthur-Merlin 游戏中，国王只能抛硬币，不仅如此，而且抛硬币的结果总会被 Merlin 知道。

这种通过 Fiat-Shamir 变换得到的 NIZK 是基于「随机预言机模型」（Random Oracle Model）。这在构造 NIZK 众多技术中非常常见，比如大家可能耳熟能详的 Bulletproofs（子弹证明），此外还有一些不那么出名的通用零知识证明方案，比如 Hyrax，Ligero，Supersonic等。我们后续会抽丝剥茧，逐一解读。

但是，However，Fiat-Shamir 变换虽然应用非常广泛，但是只能在随机预言模型下的安全性证明，而当用 Hash 函数实现随机预言的过程是否安全是缺少安全性证明的。不幸的是，事实证明，这个变换过程并**不能保证变换前可靠的协议在变换后仍然可靠**[12]。
...
细心读者也许会回看一下 Schnorr 签名，大家会发现 Schnorr 签名中的 Hash 算法似乎也漏掉了一个参数 `PK`，并不是严格的 Fiat-Shamir 变换，这被称为 Weak Fiat-Shamir 变换[9]，不过在这里并没有安全问题[4]。

最近一些年，一些学者开始试图能够严格证明安全性，目前要么引入额外的安全假设，要么针对某个特定协议进行证明，但是，似乎进展并不大。

## 交互的威力

话说在1985年，当 GMR 三人的论文历经多次被拒之后终于被 STOC’85 接受，另一篇类似的工作也同时被 STOC'85 接受，这就是来自于匈牙利罗兰大学的 László Babai，与来自以色列理工的 Shlomo Moran 两人撰写的论文『Arthur-Merlin Games: A Randomized Proof System, and a Hierarchy of Complexity Classes』，引入了 Public-coin 交互式协议（顾名思义，Verifier 只抛公共硬币）。
...
不仅如此，Babai 还大胆猜测： `AM[2]` 与 `IP` 是等价的。这是一个神奇的论断：国王很懒，他只需要通过抛一次硬币，就能成功挑战魔法师。这种方式的表达能力居然完全等价于 GMR 描述的交互式证明系统 `IP`。换句话是，Verifier 只要抛一次硬币来挑战就很完美了，能够等价于任意的交互式证明系统。果不其然，在 STOC'86 会议上，来自 S. Goldwasser 与 M. Sipser 的论文证明了这一点，`AM[poly] == IP`。

![goldwasser](img/goldwasser.png)  ![sipser](img/sipser.png)

这意味着：反复公开的「随机挑战」威力无穷，它可以等价任意的交互式证明系统。但是 `AM[poly]` 和别的计算复杂性类的关系如何，是接下来的研究热点。
...
![image-shamir](/Users/guoyu/zkt/memo/zkp-nizk/img/shamir.png)

它解释了「有效证明」这个概念的计算理论特征，并且解释了「交互式证明系统」这个概念所能涵盖的计算能力。

*注：NP 类 是 PSPACE 类的子集，前者大家比较熟悉，后者关联游戏或者下棋中的制胜策略[8]。*

而 L. Babai 于是写了一篇名为「Email and the unexpected power of interaction」（电子邮件与交互的始料未及的威力）的文章[11]，详细阐述了这一个整月的在「邮件交互」中精彩纷呈的学术竞赛，以及关于「交互证明」来龙去脉。

## 公共参考串 —— 另一种「信任根基」

在非交互式零知识证明系统中，除了采用「随机预言机」之外，还有一类的证明系统是采用「公共参考串」（Common Reference String），简称为「CRS」。CRS 是在证明者 Alice 制造 NIZK 证明之前产生的随机字符串，它必须由一个受信任的第三方来完成，同时共享给 Alice 和 验证者 Bob。

请注意，这里又出现了「第三方」！这个第三方并不直接参与证明，而只是产生一个随机字符串，保证字符串的产生过程的可信。这是另一种构建「信任根基」的方式。而产生 CRS 的过程也被称为「Trusted Setup」，是大家又爱又恨的玩意儿。

基于「随机预言模型」的 NIZK 方案的一个很大的优势就是**不需要** Trusted Setup，但是代价就是产生的证明通常都不够**简洁**。最关键的一点是，这类 NIZK 方案的安全性依赖一个似乎很虚幻的假设。

而「CRS」方案则一般不需要依赖随机预言模型，但是需要一个第三方来进行 Trusted Setup。然而在现实场景中引入第三方会让人头疼。CRS 到底是什么？Trusted Setup 的信任度何去何从？这部分内容将留给本系列的下一篇。

## 未完待续

非交互式零知识证明的信任「根基」目前来说有两种，「随机预言精灵」或「公共参考串 CRS」。后两者本质上都是引入了第三方，但是方式各不相同。并且两者都「必须」提供可以让「模拟器」利用的「后门」，以便于让模拟器在「理想世界」中具有一定的优势，但是在「现实世界」中无能为力。

非交互式零知识证明 NIZK 散发着无穷的魅力，我无时无刻不在惊叹，在过去三十多年里，诸位先驱们所探寻到的精妙结论，同时还有如此之多未知的角落，在等待智慧光芒的照射。

而学术的交流，知识的互通可能是其中必不可少的环节。

> “Everyone we interact with becomes a part of us.”  与我们交往互动的每一个人都是我们自身的一部分。 ― **Jodi Aman**



*致谢：特别感谢 丁晟超，刘巍然 的专业建议和指正，以及安比实验室小伙伴们(p0n1, even, aphasiayc, Vawheter, yghu, mr)的修改建议。*

*致谢：密码学学术竞赛的历史参考自邓老师的文章[13]。*
