...
但是，However，Fiat-Shamir 变换虽然应用非常广泛，但是只能在随机预言模型下证明安全，而当用 Hash 函数实现随机预言的过程是否安全是缺少安全性证明的。不幸的是，事实证明，这个变换过程并**不能保证变换前可靠的协议在变换后仍然可靠**[12]。
...
而 L. Babai 于是写了一篇名为「Email and the unexpected power of interaction」（电子邮件与交互的始料未及的威力）的文章[11]，详细阐述了这个月在「邮件交互」中精彩纷呈的学术竞赛，还有关于「交互证明」的来龙去脉。

## 公共参考串 —— 另一种「信任根基」

在非交互式零知识证明系统中，除了采用「随机预言机」之外，还有一类证明系统是采用「公共参考串」（Common Reference String），简称为「CRS」。CRS 是在证明者 Alice 制造 NIZK 证明之前产生的随机字符串，它必须由一个受信任的第三方来完成，同时共享给 Alice 和 验证者 Bob。
