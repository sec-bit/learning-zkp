...
第一步：请注意 Zlice 没有关于`sk`的知识，她产生一个一致性分布的随机数`c`，这时 Bob 的随机数传送带上已经预先放置了一些随机数。

![](img/schnorr-sim-2.png)


第二步：Zlice 利用一个新的「超能力」，将刚刚产生的随机数 `c` 替换掉 Bob 的随机数传送带上第一个随机数。这时候，Bob 无法察觉。

![](img/schnorr-sim-3.png)

第三步：Zlice 再次产生一个随机数 `z`，然后计算 `R'=z*G - c*PK`，并将 `R'`发送给 Bob。

![](img/schnorr-sim-4.png)

第四步：这时候Bob 会从随机数传送带上取得 `c`，并且将 `c` 发送给 Zlice。请注意这个`c` 正好就是第一步中 Zlice 产生的 `c`。

![](img/schnorr-sim-5.png)

第五步：Zlice 将第三步产生的随机数 `z` 发送给 Bob，Bob 按照 Schnorr 协议的验证公式进行验证，大家可以检查下，这个公式完美成立。
